	...
	, IndexSize(InIndexSize)
{
	...
	memset(HeapIndexes, 0xff, IndexSize * sizeof(IndexType));
}
...
void FBinaryHeap< KeyType, IndexType >::ResizeHeap(uint32 NewHeapSize)
{
	...
	if (HeapSize != 0)
	{
		memcpy(NewHeap, Heap, HeapSize * sizeof(IndexType));
		delete[] Heap;
	}
...
FORCEINLINE void FBinaryHeap< KeyType, IndexType >::Add(KeyType Key, IndexType Index)
{
	...
	if (Index >= IndexSize)
	{
		ResizeIndexes(std::max(32u, (Index + 1) * 2));
	}
