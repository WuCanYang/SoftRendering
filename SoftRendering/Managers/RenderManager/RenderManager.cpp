...
void RenderManager::Render_useQuaternion(QuaternionShader& shader, Model* model)
{
	...
	//Matrix4X4 modelMatrix = model->GetModelMatrix();
	Matrix4X4 scale = model->ScaleMatrix();
	Quaternion q = model->Rotation_Quaternion;
	Matrix4X4 translate = model->TranslateMatrix();

	std::vector<Vector3> WorldVertices;  // 顶点着色器输出, 前两个用于光照的计算
	std::vector<Vector3> WorldNormals;
	std::vector<Vector4> ClipVertices;

	shader.VertexShader(scale, q, translate, view, projection, Vertices, Normals, WorldVertices, WorldNormals, ClipVertices);
...
void RenderManager::QuaternionMode()
{
	...
	for (Model* model : *models)
	{
		Render_useQuaternion(shader, model);
	}
	displayManager->SwapBuffer();
}
...
void RenderManager::frame()
{
	...
	else if (QUATERNION) QuaternionMode();
}
