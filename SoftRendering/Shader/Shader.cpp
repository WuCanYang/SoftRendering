...
//--------------------------------------------------------------
void ShadowMapShader::VertexShader(Matrix4X4& model, Matrix4X4& view, Matrix4X4& projection,
	std::vector<Vector3>& Vertices, std::vector<Vector3>& Normals,
	std::vector<Vector3>& WorldVertices, std::vector<Vector3>& WorldNormals, std::vector<Vector4>& ClipVertices, std::vector<Vector4>& PosLightSpace)
{
	for (int i = 0; i < Vertices.size(); ++i)
	{
		Vector4 worldPos = model * Vector4(Vertices[i]);
		...
		Vector4 ClipVert = projection * view * worldPos;
		Vector4 FragPosLightSpace = lightSpaceProjection * lightSpaceView * worldPos;

		WorldVertices.push_back(Vector3(worldPos.x(), worldPos.y(), worldPos.z()));
		WorldNormals.push_back(worldNormal);
		ClipVertices.push_back(ClipVert);
		PosLightSpace.push_back(FragPosLightSpace);
	}
}

Vector3 ShadowMapShader::FragmentShader(Vector3& FragPos, Vector3& Normal, Vector2& TexCoord, Vector4& FragPosLightSpace)
{
	Vector3 Color = texture == nullptr ? Vector3(1.0f) : texture->texture2D(TexCoord.x(), TexCoord.y());
	Normal.Normalize();
