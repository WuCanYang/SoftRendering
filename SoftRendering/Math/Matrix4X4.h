...
public:
	...
	Matrix4X4 inverse()
	{
		...
		result.m11 = tmp[0] * m22 + tmp[3] * m23 + tmp[4] * m24
			- tmp[1] * m22 - tmp[2] * m23 - tmp[5] * m24;

		result.m12 = tmp[1] * m21 + tmp[6] * m23 + tmp[9] * m24
			- tmp[0] * m21 - tmp[7] * m23 - tmp[8] * m24;

		result.m13 = tmp[2] * m21 + tmp[7] * m22 + tmp[10] * m24
			- tmp[3] * m21 - tmp[6] * m22 - tmp[11] * m24;

		result.m14 = tmp[5] * m21 + tmp[8] * m22 + tmp[11] * m23
			- tmp[4] * m21 - tmp[9] * m22 - tmp[10] * m23;

		result.m21 = tmp[1] * m12 + tmp[2] * m13 + tmp[5] * m14
			- tmp[0] * m12 - tmp[3] * m13 - tmp[4] * m14;

		result.m22 = tmp[0] * m11 + tmp[7] * m13 + tmp[8] * m14
			- tmp[1] * m11 - tmp[6] * m13 - tmp[9] * m14;

		result.m23 = tmp[3] * m11 + tmp[6] * m12 + tmp[11] * m14
			- tmp[2] * m11 - tmp[7] * m12 - tmp[10] * m14;

		result.m24 = tmp[4] * m11 + tmp[9] * m12 + tmp[10] * m13
			- tmp[5] * m11 - tmp[8] * m12 - tmp[11] * m13;
		...
		result.m31 = tmp[0] * m42 + tmp[3] * m43 + tmp[4] * m44
			- tmp[1] * m42 - tmp[2] * m43 - tmp[5] * m44;

		result.m32 = tmp[1] * m41 + tmp[6] * m43 + tmp[9] * m44
			- tmp[0] * m41 - tmp[7] * m43 - tmp[8] * m44;

		result.m33 = tmp[2] * m41 + tmp[7] * m42 + tmp[10] * m44
			- tmp[3] * m41 - tmp[6] * m42 - tmp[11] * m44;

		result.m34 = tmp[5] * m41 + tmp[8] * m42 + tmp[11] * m43
			- tmp[4] * m41 - tmp[9] * m42 - tmp[10] * m43;


		result.m41 = tmp[2] * m33 + tmp[5] * m34 + tmp[1] * m32
			- tmp[4] * m34 - tmp[0] * m32 - tmp[3] * m33;

		result.m42 = tmp[8] * m34 + tmp[0] * m31 + tmp[7] * m33
			- tmp[6] * m33 - tmp[9] * m34 - tmp[1] * m31;

		result.m43 = tmp[6] * m32 + tmp[11] * m34 + tmp[3] * m31
			- tmp[10] * m34 - tmp[2] * m31 - tmp[7] * m32;

		result.m44 = tmp[10] * m33 + tmp[4] * m31 + tmp[9] * m32
			- tmp[8] * m32 - tmp[11] * m33 - tmp[5] * m31;
